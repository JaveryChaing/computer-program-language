...
- #### **ActiveMQ**
  ...
  > 2. 发起业务请求，在数据库A中创建数据，并在事件表t_event中记录该事件，提交该事务
  > 3. 在业务中开启动定时任务查询数据库A中t_event中所有process为NEW的记录，存在记录则向ActiveMQ发送消息，消息内容为t_event中content参数，消息发送成功后把该条数据状态改为Published，提交事务
  > 4. 消费者接收到ActiveMQ消息后在数据库B中t_event中新增一条process为Publish的记录，保存消息成功后返回并提交事务
  > 5. 在业务系统中开启
  >
  > 
